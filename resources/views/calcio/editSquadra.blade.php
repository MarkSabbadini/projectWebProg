...
@section('body')
    <div class="container my-5">
        ...
        <form method="POST" action="{{ route('squadra.update', $squadra->id) }}">
            ...
            <div class="mb-4">
                ...
                <input type="text" class="form-control" name="nome_squadra" id="nomeSquadraInput"
                    value="{{ $squadra->nome }}" required>
            ...
            <div id="calciatori-container">
                @foreach($squadra->calciatori as $i => $c)
                    <div class="row mb-3 calciatore-row align-items-center" data-index="{{ $i }}">
                        <div class="col-md-3">
                            <input type="text" name="calciatori[{{ $i }}][nome]" class="form-control nome-calciatore"
                                value="{{ $c->nome }}" placeholder="Nome" required>
                        </div>
                        <div class="col-md-3">
                            <input type="text" name="calciatori[{{ $i }}][cognome]" class="form-control cognome-calciatore"
                                value="{{ $c->cognome }}" placeholder="Cognome" required>
                            <div class="invalid-feedback d-none">⚠️ Calciatore già assegnato ad un'altra squadra.</div>
                        </div>
                        <div class="col-md-2">
                            <input type="number" name="calciatori[{{ $i }}][numero]" class="form-control"
                                value="{{ $c->numero }}" placeholder="Numero" required>
                        </div>
                        <div class="col-md-3">
                            <input type="text" name="calciatori[{{ $i }}][ruolo]" class="form-control" value="{{ $c->ruolo }}"
                                placeholder="Ruolo" required>
                        </div>
    ...
    <script>
        ...
        $('#aggiungi-calciatore').on('click', function () {
            ...
            const row = `
                <div class="row mb-3 calciatore-row align-items-center" data-index="${index}">
                    ...
                    <div class="col-md-1 text-end">
                        <button type="button" class="btn btn-outline-danger btn-sm rimuovi-calciatore" title="Rimuovi">❌</button>
                    </div>
                </div>
            `;
        ...
        // Rimuovo un calciatore dinamicamente
